...
wb_clean <- function(df) {
  ...
  clean1 <- df %>%
    dplyr::mutate(ind = dplyr::case_when(indicatorID == "SH.STA.STNT.ZS" ~ "stunt.u5",
                                         ...
                                         stringr::str_detect(indicatorID, "LIT.BE|LIT.1")  ~ "adult.profinumeracy",
                                         stringr::str_detect(indicatorID, "NUM.BE|NUM.1")  ~ "adult.profiliteracy",
...
weights_clean <- function(df) {
  ...
  cleaned <- dplyr::bind_rows(clean1, clean2) %>%
    dplyr::group_by(year, wt_var) %>%
    dplyr::filter(!is.na(wt_value)) %>%
    dplyr::mutate(wt_value_z = (wt_value - mean(wt_value))/sd(wt_value))
  ...
  wide_data <- df %>%
    dplyr:: mutate(value = dplyr::case_when(stringr::str_detect(ind, stringr::regex("wpia|gpia|lpia|sal\\.rel", ignore_case = TRUE)) ~ round(value, digits = 2),
                   ...
                   value = dplyr::case_when(entity == "country" & stringr::str_detect(ind, stringr::regex("esd|bully|attack", ignore_case = TRUE)) & value == 1 ~ "Low",
                                            ...
                                            entity == "country" & stringr::str_detect(ind, stringr::regex("attack", ignore_case = TRUE)) & value == 3 ~ "Extreme",
